...
local function HopeWidgetLevel(hopeFrame, offset)
    return hopeFrame:GetFrameLevel() + (offset or 5)
end
...
function BG.TCOCleanupHopeOverlays(FB)
    if not FB or not BG.HopeFrame or not BG.HopeFrame[FB] or not ns.HopeMaxn or not ns.HopeMaxn[FB] then
        return
    end
...
local function EnsureHopeSavedStructures(FB)
    if not FB or not ns.HopeMaxn or not ns.HopeMaxn[FB] or not ns.HopeMaxb or not ns.HopeMaxb[FB] then
        return nil
    end
    BiaoGe.Hope = BiaoGe.Hope or {}
    BiaoGe.Hope[RealmID] = BiaoGe.Hope[RealmID] or {}
    BiaoGe.Hope[RealmID][BG.playerName] = BiaoGe.Hope[RealmID][BG.playerName] or {}
    BiaoGe.Hope[RealmID][BG.playerName][FB] = BiaoGe.Hope[RealmID][BG.playerName][FB] or {}
    local root = BiaoGe.Hope[RealmID][BG.playerName][FB]
    for n = 1, ns.HopeMaxn[FB] do
...
function BG.HopeUI(FB)
    ...
    if not hopeFrame or (not hopeFrame.isTCOHope and not hopeFrame.isBGPHope) then
        return false
    end
    ...
    if hopeFrame.bgpHopeGridBuilt then
        local cell = BG.HopeFrame
            and BG.HopeFrame[FB]
            and BG.HopeFrame[FB]["nandu1"]
            and BG.HopeFrame[FB]["nandu1"]["boss1"]
            and BG.HopeFrame[FB]["nandu1"]["boss1"]["zhuangbei1"]
        if cell and cell.GetObjectType and cell:GetObjectType() == "EditBox" and cell:GetParent() == hopeFrame then
            return true
        end
        hopeFrame.bgpHopeGridBuilt = nil
    end
        ...
        for b = 1, ns.HopeMaxb[FB], 1 do
            for i = 1, ns.HopeMaxi, 1 do
                ...
                do
                    ...
                    bt:SetFrameLevel(HopeWidgetLevel(hopeFrame, 5))
                    StyleHopeEditBox(bt)
                    if i == 1 then
                    ...
                    local hopeData = BiaoGe.Hope
                        ...
                        and BiaoGe.Hope[RealmID][BG.playerName]
                        and BiaoGe.Hope[RealmID][BG.playerName][FB]
                        and BiaoGe.Hope[RealmID][BG.playerName][FB]["nandu" .. n]
                        and BiaoGe.Hope[RealmID][BG.playerName][FB]["nandu" .. n]["boss" .. b]
                    ...
                    if i == 1 then
                        framedown = bt
                        -- if n == 1 or n == 3 and b == ns.HopeMaxb[FB] then
                        if b == ns.HopeMaxb[FB] then
                            framedownH = bt
                        end
                    end
                    -- 创建已掉落文本
                    ...
                    bt:SetScript("OnTextChanged", function(self)
                        ...
                        BiaoGe.Hope[RealmID][BG.playerName] = BiaoGe.Hope[RealmID][BG.playerName] or {}
                        BiaoGe.Hope[RealmID][BG.playerName][FB] = BiaoGe.Hope[RealmID][BG.playerName][FB] or {}
                        BiaoGe.Hope[RealmID][BG.playerName][FB]["nandu" .. n] =
                            BiaoGe.Hope[RealmID][BG.playerName][FB]["nandu" .. n] or {}
                        BiaoGe.Hope[RealmID][BG.playerName][FB]["nandu" .. n]["boss" .. b] =
                            BiaoGe.Hope[RealmID][BG.playerName][FB]["nandu" .. n]["boss" .. b] or {}
                        if itemText ~= "" then
                            BiaoGe.Hope[RealmID][BG.playerName][FB]["nandu" .. n]["boss" .. b]["zhuangbei" .. i] = itemText
                        else
                            BiaoGe.Hope[RealmID][BG.playerName][FB]["nandu" .. n]["boss" .. b]["zhuangbei" .. i] = nil
                        end
                    end)
                    -- 点击
                    bt:SetScript("OnMouseDown", function(self, button)
                        ...
                        if button == "LeftButton" then
                            ...
                            -- 装备列表仅在 OnEditFocusGained 打开，避免与 MouseDown 重复创建
                        end
                    end)
                        ...
                        if not IsModifierKeyDown() then
                            if enter == "UP" then -- 上↑
                                if BG.HopeFrame[FB]["nandu" .. n]["boss" .. b - 1] and
                                    ...
                                    BG.HopeFrame[FB]["nandu" .. nn]["boss" .. ns.HopeMaxb[FB]]["zhuangbei" .. i]:SetFocus()
                                end
                            ...
                            elseif enter == "LEFT" then -- 左←
                                if BG.HopeFrame[FB]["nandu" .. n]["boss" .. b]["zhuangbei" .. i - 1] then
                                    ...
                                    BG.HopeFrame[FB]["nandu" .. nn]["boss" .. b]["zhuangbei" .. ns.HopeMaxi]:SetFocus()
                                end
    ...
    -- 格子建完即标记，通报区失败也不影响主网格
    hopeFrame.bgpHopeGridBuilt = true

    if not BG.IsRetail and LibBG and BG.CreateButton and BG.dropDownToggle then
        ------------------通报心愿------------------
        do
            local xinyuan
            ...
            local reportBtn
            for n = 1, ns.HopeMaxn[FB] do
                local entry = xinyuan[n] or xinyuan[1]
                ...
                bt:SetText(entry.name1)
                bt:SetFrameLevel(HopeWidgetLevel(hopeFrame, 3))
                reportBtn = bt
    ...
    -- 更新心愿
    if not hopeFrame.bgpHopeShowHooked then
        hopeFrame.bgpHopeShowHooked = true
        hopeFrame:HookScript("OnShow", function(self)
        for n = 1, ns.HopeMaxn[FB] do
            for b = 1, ns.HopeMaxb[FB] do
                for i = 1, ns.HopeMaxi do
                    local bt = BG.HopeFrame[FB]["nandu" .. n]["boss" .. b]["zhuangbei" .. i]
                    if bt and bt:GetText() == "" then
                        for ii = i, ns.HopeMaxi do
...
----------导出导入心愿心愿----------
function BG.BGP_LayoutHopeExportImport(show)
    if not BG.ButtonImportHope or not BG.ButtonExportHope or not BG.MainFrame then
        return
    end
    local mf = BG.MainFrame
    local titleLevel = (mf.CloseButton and mf.CloseButton:GetFrameLevel() or mf:GetFrameLevel()) + 2
    if show then
        BG.ButtonImportHope:SetParent(mf)
        BG.ButtonImportHope:ClearAllPoints()
        -- 标题栏右侧，关闭按钮左边
        BG.ButtonImportHope:SetPoint("TOPRIGHT", mf, "TOPRIGHT", -68, -4)
        BG.ButtonImportHope:SetFrameLevel(titleLevel)
        BG.ButtonImportHope:Show()

        BG.ButtonExportHope:SetParent(mf)
        BG.ButtonExportHope:ClearAllPoints()
        BG.ButtonExportHope:SetPoint("RIGHT", BG.ButtonImportHope, "LEFT", -7, 0)
        BG.ButtonExportHope:SetFrameLevel(titleLevel)
        BG.ButtonExportHope:Show()
    else
        BG.ButtonImportHope:Hide()
        BG.ButtonExportHope:Hide()
...
function BG.HopeDaoChuUI()
    if BG.ButtonImportHope then
        BG.BGP_LayoutHopeExportImport(BG.HopeMainFrame and BG.HopeMainFrame:IsVisible())
        return
    end

    local width_jiange = -7
    local hideFrameTbl = {}
    local parent = BG.MainFrame
            ...
            for _, _FB in ipairs(BG.FBtable) do
                if FB == _FB then
                    ...
                    for _, v in ipairs({ strsplit(",", allboss) }) do
                        ...
                        if n and b and n <= ns.HopeMaxn[FB] and b <= ns.HopeMaxb[FB] then
                            local i = 1
                            for ii = 2, #text do
                                ...
                                if itemID then
                                    ...
                                    if BG.HopeFrame[FB]["nandu" .. n]["boss" .. b]["zhuangbei" .. i] then
                                        ...
                                        item:ContinueOnItemLoad(function()
                                            ...
                                            BiaoGe.Hope[RealmID][BG.playerName][FB]["nandu" .. n]["boss" .. b]["zhuangbei" .. _i] = link
                                        end)
                                        i = i + 1
                                    end
                                end
    ...
    do
        ...
        bt:SetPoint("TOPRIGHT", parent, "TOPRIGHT", -68, -4)
        ...
        bt:SetScript("OnClick", function(self)
            ...
            if not self.bg then
                ...
                do
                    ...
                    bg:SetPoint("TOPRIGHT", parent, "TOPRIGHT", -20, -28)
                    bg:SetSize(250, 250)
...
function BG.SetHope(link, FB, isBiaoGe)
    ...
    for i = 1, ns.HopeMaxi do
        ...
        if hope and hope:GetText() == "" then
            ...
            BiaoGe.Hope[RealmID][BG.playerName][FB]["nandu" .. n]["boss" .. b]["zhuangbei" .. i] = link
            ...
            for b = 1, ns.HopeMaxb[FB] do
                for i = 1, ns.HopeMaxi do
                    ...
                    local saved = BiaoGe.Hope
                        ...
                        and BiaoGe.Hope[RealmID][BG.playerName]
                        and BiaoGe.Hope[RealmID][BG.playerName][FB]
                        and BiaoGe.Hope[RealmID][BG.playerName][FB]["nandu" .. n]["boss" .. b]["zhuangbei" .. i]
...
function BG.TCOShowHopeLootList(owner)
    ...
    if BG.CreateCloseButton then
        ...
        f.CloseButton:SetScript("OnClick", function()
            f:Hide()
            if BG.FrameZhuangbeiList == f then
                BG.FrameZhuangbeiList = nil
            end
