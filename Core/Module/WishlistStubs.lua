...
-- 保留 BGLite 原清空表格逻辑，Plus 只扩展 hope 类型
local origClearBiaoGe = BG.ClearBiaoGe
...
function BG.ClearBiaoGe(_type, FB)
    if _type == "hope" then
        return BG.BGP_ClearHope(FB)
    end
    if origClearBiaoGe then
        return origClearBiaoGe(_type, FB)
    end
end
